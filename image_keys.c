...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/09/18 17:32:31 by qcraghas          #+#    #+#             */
/*   Updated: 2020/09/18 17:32:33 by qcraghas         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	wasd(t_cub *d, int key)
{
	...
	if (key == DOWN)
		...
		angle = d->pl.angle;
	x = d->pl.pos_x + move * cos(angle);
	y = d->pl.pos_y + move * sin(angle);
	wall_sliding(d, x, y);
}
...
int		close_win(t_cub *data)
{
	...
	exit(ft_errors(RED_BUTTON_EXIT));
}
